...
def predict_liveness(X_deep, X_micro, return_dict=False):
    ...
    if deep_conf_fake >= 0.4 or micro_conf_negative >= 0.4:
        final_decision = fusion_result

    if return_dict:
        return {
            "deepfake_label": ["Real", "Fake"][deep_class],
            "deepfake_conf": round(float(deep_pred[deep_class]), 4),
            "micro_label": ["Positive", "Negative", "Surprise"][micro_class],
            "micro_conf": round(float(micro_pred[micro_class]), 4),
            "fusion_label": fusion_result,
            "fusion_conf": round(float(max(fusion_model.predict_proba(input_scaled)[0])), 4),
